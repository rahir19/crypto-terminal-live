...
@app.callback(
    ...
    [Input('about-link-btn', 'n_clicks'), Input('contact-link-btn', 'n_clicks')],
    prevent_initial_call=True
)
def toggle_modal(about_clicks, contact_clicks):
    ...
    if ctx_id == 'about-link-btn':
        ...
        content = html.Div(className='contact-info-box', children=[
            ...
            html.Div(className='contact-item', children=["Name: ", html.Span("Raghav Ahir Yaduvanshi", style={'fontWeight':'bold'})]),
            html.Div(className='contact-item', children=["Phone: ", html.Span("6266649445", style={'fontWeight':'bold'})]),
             html.Div(className='contact-item', children=[
                 "GitHub: ", 
                 html.A("https://github.com/rahir19", href="https://github.com/rahir19", target="_blank", className='contact-link')
             ]),
            html.Div(className='contact-item', children=[
                 "LinkedIn: ", 
    ...
    if content:
        return html.Div(className='modal-overlay', children=[
            html.Div(className='modal-content', children=[
                 html.Span("×", id='modal-close-btn', className='modal-close'),
                 content
            ])
        ])
    return None
...
def update_overview(n, selected_symbol, tf_data, ws_data):
    ...
    try:
        ...
        for sym in TRACKER_SYMBOLS:
            if sym in all_tickers:
                d = all_tickers[sym]; name = SYMBOL_MAP.get(sym, sym); p_change = d['percentage']
